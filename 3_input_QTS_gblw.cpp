...
#define m 10 // gate count
#define output_len_6  4
#define output_len_8  3
#define output_len_10  2
using namespace std;
...
// int output_3bit[output_len_8][8] ={{3,6,2,5,7,1,0,4}, {4,3,0,2,7,5,6,1}, {7,5,4,2,0,1,6,3}};                    
int output_3bit[output_len_10][8]={{0,1,2,4,3,5,6,7}, {1,2,7,5,6,3,0,4}};
...
void update()
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        {
            //gb.lw compare
            if (gb[j][i] != X[lw_index][j][i]) //not same -> update
            {
                Q[j][i][gb[j][i]] += DELTA;
...
int main()
{
    for(int gate = 0; gate < output_len_10; gate++)
    {
        srand(114);
